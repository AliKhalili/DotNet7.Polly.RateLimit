...
public class SlidingWindow_AsyncRateLimitSyntaxTest : AsyncRateLimitSyntaxBaseTest
{
    ...
    {
        // Arrange
        var invalidSyntax = () => DotNet7Policy.SlidingWindowRateLimitAsync(options: null!);
    ...
    {
        // Arrange
        var invalidSyntax = () => DotNet7Policy.SlidingWindowRateLimitAsync(configureOptions: null!);
    ...
    {
        // Arrange
        var rateLimiter = DotNet7Policy.SlidingWindowRateLimitAsync(new SlidingWindowRateLimiterOptions
        {
            PermitLimit = 1,
            SegmentsPerWindow = 1,
    ...
    {
        // Arrange
        var rateLimiter = DotNet7Policy.SlidingWindowRateLimitAsync(
            option =>
        {
    ...
    public override async void Given_limiter_with_N_permit_throw_rate_limit_exception_for_N_plus_1_th_request_and_acquire_for_next_N_th_after_replenishment(int permitLimit)
    {
        ...
        var rateLimiterPolicy = DotNet7Policy.SlidingWindowRateLimitAsync(
            option =>
            {
                option.PermitLimit = permitLimit;
                option.SegmentsPerWindow = 1;
