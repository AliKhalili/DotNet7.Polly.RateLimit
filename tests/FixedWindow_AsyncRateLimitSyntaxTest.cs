...
public class FixedWindow_AsyncRateLimitSyntaxTest : AsyncRateLimitSyntaxBaseTest
{
    ...
    {
        // Arrange
        var invalidSyntax = () => DotNet7Policy.FixedWindowRateLimitAsync(options: null!);
    ...
    {
        // Arrange
        var invalidSyntax = () => DotNet7Policy.FixedWindowRateLimitAsync(configureOptions: null!);
    ...
    {
        // Arrange
        var rateLimiter = DotNet7Policy.FixedWindowRateLimitAsync(new FixedWindowRateLimiterOptions
        {
            PermitLimit = 1,
    ...
    {
        // Arrange
        var rateLimiter = DotNet7Policy.FixedWindowRateLimitAsync(
            option =>
        {
    ...
    public override async void Given_limiter_with_N_permit_throw_rate_limit_exception_for_N_plus_1_th_request_and_acquire_for_next_N_th_after_replenishment(int permitLimit)
    {
        ...
        var rateLimiterPolicy = DotNet7Policy.FixedWindowRateLimitAsync(
            option =>
            {
                option.PermitLimit = permitLimit;
                option.QueueLimit = 0;
